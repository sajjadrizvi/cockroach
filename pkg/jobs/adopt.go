...
func (r *Registry) resumeJob(ctx context.Context, jobID jobspb.JobID, s sqlliveness.Session) error {
	...
	// TODO(sajjad): We should call addAdoptedJob in the adopt-loop's goroutine.
	// Otherwise, we can end-up with multiple resumers due to a race. For example,
	// if this goroutine stalls for some reason until, the next adopt loop will
	// start another resumer in processClaimedJobs.
...
func (r *Registry) servePauseAndCancelRequests(ctx context.Context, s sqlliveness.Session) error {
	return r.db.Txn(ctx, func(ctx context.Context, txn *kv.Txn) error {
		...
		for _, row := range rows {
			...
			case StatusPaused:
				// TODO(sajjad): Do not unregister the job here. Instead, only cancel the context.
				// Otherwise, we can have multiple resumers started if the job is unpaused before
				// the current resumer finishes. Only the currently executing resumer should
				// unregister because the adoptedJobs map is serving like an exclusive lock.
				r.unregister(id)
			...
			case StatusReverting:
				if err := job.Update(ctx, txn, func(txn *kv.Txn, md JobMetadata, ju *JobUpdater) error {
					// TODO(sajjad): Do not unregister, instead cancel the resumer. See explanation
					// above in the StatusPaused case.
					r.unregister(id)
					...
					if r.settings.Version.IsActive(ctx, clusterversion.RetryJobsWithExponentialBackoff) {
						...
						ju.UpdateRunStats(1 /* numRuns */, r.clock.Now().GoTime() /* lastRun */)
					}
					return nil
				}); err != nil {
