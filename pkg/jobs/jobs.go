...
func (j *Job) reverted(
	...
	return j.Update(ctx, txn, func(txn *kv.Txn, md JobMetadata, ju *JobUpdater) error {
		...
		if j.registry.settings.Version.IsActive(ctx, clusterversion.RetryJobsWithExponentialBackoff) {
			...
			// If the job was failed as compared to cancel or unpause code paths.
			if md.Status != StatusReverting {
...
func (sj *StartableJob) CleanupOnRollback(ctx context.Context) error {
	...
	// Otherwise, we can have multiple resumers started if the job is paused and unpaused
	// before the current resumer finishes. We should unregister only from the currently
	// executing resumer.
